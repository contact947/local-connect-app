...
## フェーズ5: 初期起動フロー・RBAC・UI刷新

### フェーズ1: 初期起動フローと会員登録画面の刷新
- [x] 初期起動判定の確認（未ログイン時に会員登録画面表示）
- [x] 会員登録画面のUI改善（新規登録フォーム＋ログイン遷移ボタン）
- [x] ログイン画面の改善

### フェーズ2: RBAC権限管理システムの実装
- [x] Firestore スキーマ確認（role フィールド）
- [x] 権限チェック関数の実装（hasPermission, checkRole）
- [x] admin/planner/user ロール定義
- [x] デフォルト role: "user" の付与ロジック確認

### フェーズ3: ホームタブの構成変更
- [x] ホームタブの順序確認（トップニュース→参加予定イベント→地域ニュース→地域イベント）
- [x] ユーザー住所ベースのフィルタリング確認

### フェーズ4: ニュース/イベントタブのセグメント切り替え
- [x] SegmentedControl コンポーネント実装
- [x] ニュースタブに「全国」「地域」セグメント追加
- [x] イベントタブに「全国」「地域」セグメント追加
- [x] 地域フィルタリングロジック実装

### フェーズ5: アカウント画面の刷新
- [x] リスト形式UI実装（アカウントID、氏名、年齢、住所、性別、職業）
- [x] Firestore連携（表示・編集）
- [x] プロフィール編集画面の改善
...
### フェーズ1: 都道府県・市区町村の JSON ファイル作成
- [x] constants/prefectures.json 作成（都道府県と市区町村の階層データ）
- [x] TypeScript 型定義作成（Prefecture, City）
### フェーズ2: アカウントタブの UI/UX 改善
- [x] クリップボード機能（アカウントID・メールアドレスの長押しコピー）
- [x] 性別選択をボタン式に変更（男性・女性・その他）
- [x] 住所選択をプルダウン化（都道府県 → 市区町村のカスケード）
- [x] 保存/キャンセルボタンの UI 改善（塗りつぶし vs アウトライン）
- [x] 確認ポップアップの実装（保存・キャンセル両方）

### フェーズ3: ニュース/イベントタブのセグメント変更
- [x] ニュースタブ: 「地域のニュース」「全国のニュース」セグメント実装
- [x] イベントタブ: 「地域のイベント」「全国のイベント」セグメント実装
- [x] Firestore スキーマ確認（prefecture フィールド有無）
- [x] フィルタリングロジック実装（prefecture フィールド or 住所文字列抽出）
- [x] カテゴリフィルターの維持（セグメント内でのカテゴリ絞り込み）

### フェーズ4: ホームタブのカード型 UI 実装
- [x] ニュース/イベントの表示形式をカード型に変更
- [x] 各セクション見出しの維持
- [x] 画像・タイトル・日付をカード内に配置

### フェーズ5: ギフトタブの GPS 住所情報連携
- [x] expo-location パッケージのインストール
- [x] GPS 権限リクエスト実装
- [x] Firestore ギフトデータの latitude/longitude フィールド確認
- [x] ダミーデータの追記（GPS 取得不可時のテスト用）
- [x] GPS 距離計算ロジック実装
- [x] フォールバック（ユーザー市区町村ベース）実装
- [x] ギフト一覧の最大表示数を 20 件に制限

### フェーズ6: 動作確認とテスト
- [x] 開発サーバーの状況確認
- [x] 各タブの動作確認
- [x] GPS 機能のテスト（実機またはシミュレーター）
